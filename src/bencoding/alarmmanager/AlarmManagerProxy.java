...
public class AlarmManagerProxy extends KrollProxy {
        ...
        if (isRepeating && !intent.hasExtra("notification_repeat_ms")) {
            ...
            if (Build.VERSION.SDK_INT < 23) {
                if (Build.VERSION.SDK_INT >= 19) {
                    utils.debugLog("Setting Alarm for a single run (setExact)");
                    am.setExact(AlarmManager.RTC_WAKEUP, calendar.getTimeInMillis(), sender);
                } else {
                    utils.debugLog("Setting Alarm for a single run (set)");
                    am.set(AlarmManager.RTC_WAKEUP, calendar.getTimeInMillis(), sender);
                }
            } else {
                utils.debugLog("Setting Alarm for a single run (setExactAndAllowWhileIdle)");
    ...
    public void addAlarmService(@SuppressWarnings("rawtypes") HashMap hm) {@
        ...
        String DATE_FORMAT = "yyyy-MM-dd HH:mm:ss";
        SimpleDateFormat sdf = new SimpleDateFormat(DATE_FORMAT);
        utils.debugLog("Creating Alarm Notification for: " + sdf.format(calendar.getTime()) + " (" + requestCode + ")");
